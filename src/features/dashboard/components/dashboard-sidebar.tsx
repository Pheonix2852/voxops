...
import { type LucideIcon, Home, LayoutGrid, AudioLines, Headphones, Volume2, Settings } from "lucide-react";
          ...
          {items.map((item) => (
            <SidebarMenuItem key={item.title}>
              <SidebarMenuButton
                ...
                tooltip={item.title}
                className="h-9 px-3 py-2 text-[13px] font-medium tracking-tight border border-transparent data-[active = true]:border-border data-[active = true]:shadow-[0px_1px_1px_0px_rgba(44,54,53,0.03), inset(0px_0px_0px_2px_white]"
              >
                {item.url ? (
                  <Link href={item.url}>
                    <item.icon />
...
export function DashboardSidebar() {
  ...
  const otherMenuItems: MenuItem[] = [
    ...
    {
      title: "Help & Support",
      url: "mailto:kumbhakarsagnik@gmail.com",
      icon: Headphones,
    },
  ];
    ...
    <Sidebar collapsible="icon">
      <SidebarHeader className="flex flex-col gap-4 pt-4">
        <div className="flex items-center gap-2 pl-1 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:pl-0">
          <Image src="/logo.png" alt="VoxOps" width={32} height={32} className="rounded-sm" />
          <span className="group-data-[collapsible=icon]:hidden font-semibold text-2xl tracking-tighter text-foreground">
            VoxOps
          </span>
            ...
            <OrganizationSwitcher
              ...
              appearance={{
                elements: {
                  ...
                  organzizationSwitcherTrigger:
                    "w-full! justify-between! bg-white! border! border-border! rounded-md! pl-1! py-2! pr-1! gap-3! group-data-[collapsible=icon]:w-auto! group-data-[collapsible=icon]:p-1! shadow-[0px_1px_1px_0px_rgba(44,54,53,0.03)",
                  organizationPreview: "gap-2!",
                  organizationPreviewAvatarBox: "size-6! rounded-sm!",
                  organizationPreviewTextContainer:
                    "text-xs! tracking-tight! font-medium! text-foreground! group-data-[collapsible=icon]:hidden!",
                  oranizationPReviewMainIDentifier: "text-[13px]!",
